...
const UserProfile: React.FC = () => {
  ...
  const [profile, setProfile] = useState<ProfileWithEntriesResponse | null>(
    null,
  );
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(`${apiUrl}/user/${username}/profile`);
      ...
      if (tags.length > 0) {
        setFilteredEntries(
          entries.filter((entry) =>
            tags.some(
              (tag) => entry.tags.some((entryTag) => entryTag.name === tag), // If entry.tags is an array of TagProp with name field
            ),
          ),
        );
      } else {
  ...
  const downloadResume = async () => {
    ...
    alert(
      "Integrating a safe privacy protected resume upload, please come back later.",
    );
  };
    ...
    <div className="bg-gray-100 p-4 rounded-lg shadow-md mb-4">
        ...
        <div className="container max-w-screen-xl mx-auto px-4">
          ...
          <div className="text-center">
            ...
            <section className="py-4">
              ...
              />{" "}
              {/* Using FileUpload Component here */}
            </section>
      ...
      <section className="py-6 md:py-4 mb-3">
        <UserJournalSection
          ...
          filteredEntries={[]}
          setFilteredEntries={function (entries: JournalEntryProp[]): void {
