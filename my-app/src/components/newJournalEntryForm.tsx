...
const NewJournalEntryForm: React.FC<NewJournalEntryFormProps> = ({
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          alert(
            `Error ${error.response.status}: ${error.response.data.message || "An error occurred on the server."}`,
          );
  ...
  const handleTagChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uniqueTags = newQuery
      ...
      .map((tag) => tag.trim())
      .filter(
        (name) => name && !tags.some((tag) => tag.name.toLowerCase() === name),
      );
    ...
    if (uniqueTags.length > 0) {
      try {
        ...
        const response = await axios.get(
          `${apiUrl}/tags/search?query=${newQuery}`,
        );
